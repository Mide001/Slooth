...
export class GetTokenPairsParameters extends createToolParameters(
  z.object({
    tokenAddresses: z
      .array(z.string())
      .max(30)
      .describe("List of up to 30 token addresses"),
  })
) {}

export class TokenInfoQueryParams extends createToolParameters(
  z.object({
    chainId: z.string().describe("The chain id of the token address"),
    tokenAddress: z.string().describe("The token address of the token"),
  })
) {}
