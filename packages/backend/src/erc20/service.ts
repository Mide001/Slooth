...
import {
  ...
  GetTokenInfoByTokenAddressParameters,
...
export class Erc20Service {
    ...
    try {
      ...
      if (existingToken) {
        return {
          ...
          contractAddress: parameters.tokenAddress,
          decimals: existingToken.decimals, // Fixed: was using name instead of decimals
          name: existingToken.name, // Added missing name field
        };
      }
      ...
      const [nameResult, symbolResult, totalSupplyResult, decimalsResult] =
        await Promise.all([
          walletClient.read({
            ...tokenContract,
            functionName: "name",
      ...
      const totalSupply = String(totalSupplyResult.value);
      ...
      throw Error(`Failed to fetch token info: ${error}`);
    }
  }
  ...
  async getTokenBalance(
    ...
    try {
      const resolvedAddress = await walletClient.resolveAddress(
        parameters.wallet
      );
